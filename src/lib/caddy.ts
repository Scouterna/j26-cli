...
function resolveUpstream(entry: ServiceWithConfig): {upstream: string; urlBasePath?: string} {
  const {service, localConfig} = entry
  switch (localConfig.mode) {
    case 'cloud': {
      if (!('cloudUrl' in service) || !service.cloudUrl) {
        throw new Error(`Service "${service.name}" has no cloudUrl but is configured for cloud mode.`)
      }
      ...
      const urlBasePath = url.pathname === '/' ? undefined : url.pathname.replace(/\/$/, '')
    ...
    case 'local': {
      if (!localConfig.hostPort) {
        throw new Error(`Service "${service.name}" is in dev mode but has no hostPort configured.`)
      }
    ...
    case 'skip': {
      ...
      throw new Error(`Service "${service.name}" is in skip mode and should not be passed to generateCaddyfile.`)
    }
  }
}
