...
export default class Status extends BaseCommand<typeof Status> {
  ...
  public async run(): Promise<{services: ServiceStatus[]} | void> {
    const manifest = await loadManifest(this.flags.config)
    const localConfig = await loadLocalConfig()

    const statuses: ServiceStatus[] = manifest.services.map((service) => {
      const cfg: ServiceLocalConfig = localConfig.services[service.name] ?? {mode: 'cloud'}
      return {
        name: service.name,
        path: service.path,
        mode: cfg.mode,
    ...
    for (const service of Object.values(localConfig.localServices ?? {})) {
      const cfg: ServiceLocalConfig = localConfig.services[service.name] ?? {mode: 'local'}
      statuses.push({
        name: `${service.name} *`,
        path: service.path,
        mode: cfg.mode,
...
function buildDetails(cfg: ServiceLocalConfig): string {
  switch (cfg.mode) {
    ...
    case 'local':
      return cfg.hostPort ? `forwarded to localhost:${cfg.hostPort}` : 'local (run j26 config to set port)'
    case 'skip':
      return 'excluded'
  }
}
