...
Run \`j26 config\` first to choose which services to run locally.
  `.trim();
	...
	public async run(): Promise<void> {
		...
		for (const service of manifest.services) {
			...
			if (!existing && !isFirstRun) {
				this.warn(
					`New service "${service.name}" not yet configured — defaulting to cloud. Run \`j26 config\` to change.`,
				);
			}

			servicesWithConfig.push({
				service,
				localConfig: existing ?? { mode: "cloud" },
			});
		}
		...
		for (const service of Object.values(localConfig.localServices ?? {})) {
			...
			servicesWithConfig.push({
				service,
				localConfig: { mode: "local", hostPort, env: existing?.env },
			});
		}
		...
		if (isFirstRun) {
			...
			this.log(
				"Created .j26.local.yaml — run `j26 config` to customise service modes.",
			);
		}

		// Skip services are excluded from the generated Caddyfile / docker-compose
		const activeServices = servicesWithConfig.filter(
			({ localConfig }) => localConfig.mode !== "skip",
		);
		...
		if (this.flags.detach) {
			...
			this.log("    Tips:");
			this.log(
				"      Run `j26 trust-ca` once to avoid browser certificate warnings.",
			);
	...
	private async generateFiles(services: ServiceWithConfig[]): Promise<void> {
		...
		this.log(
			`Generated ${GENERATED_DIR}/Caddyfile and ${GENERATED_DIR}/docker-compose.yml`,
		);
	}
