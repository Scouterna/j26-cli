...
import type {LocalConfig, LocalOnlyService, ManifestService, ServiceLocalConfig, ServiceMode} from '../lib/schemas.ts'
  ...
  public async run(): Promise<void> {
    ...
    while (true) {
      ...
      if (selected === '__add_local__') {
        ...
        localConfig.localServices = {...(localConfig.localServices ?? {}), [newService.name]: newService}
        localConfig.services[newService.name] = {mode: 'local', hostPort: newService.port}
        continue
      }

      const manifestService = manifestServices.find(s => s.name === selected)
      const localOnlyService = localOnlyServices.find(s => s.name === selected)
      const existing = localConfig.services[selected]

      if (manifestService) {
        const result = await configureManifestService(manifestService, existing)
        ...
        if (action === 'remove') {
          const {[selected]: _removed, ...rest} = localConfig.localServices ?? {}
          localConfig.localServices = rest
          delete localConfig.services[selected]
        } else {
          localConfig.services[selected] = await configureLocalOnlyService(localOnlyService, existing)
        }
      }
    }
...
async function promptNewLocalService(
  ...
  const name = await input({
    ...
    validate(value) {
      ...
      if (takenNames.has(value.trim())) return `"${value.trim()}" is already in use`
      return true
    },
  })
...
async function configureManifestService(
  ...
  const mode = await select<ServiceMode>({
    ...
    choices: [
      ...
      {name: 'Local   — forward to a server running on your machine', value: 'local'},
  ...
  if (mode === 'local') {
    hostPort = await promptHostPort(service.name, existing?.hostPort ?? service.port)
  }
...
async function configureLocalOnlyService(
  ...
  const hostPort = await promptHostPort(service.name, existing?.hostPort ?? service.port)
  return {mode: 'local', hostPort, env: existing?.env}
}

async function promptHostPort(serviceName: string, defaultPort: number): Promise<number> {
